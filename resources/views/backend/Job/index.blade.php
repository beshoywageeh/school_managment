    ...
    <div class="card-body">
            ...
            <form action="{{route('jobs.store')}}" method="post">
                ...
                <div class="flex flex-col gap-1 items-center w-full md:flex-row md:gap-2">
                    ...
                    <div class="w-full md:w-1/3">
                        <input class="input" id="job_name" name="job_name" type="text" value="{{old('job_name')}}">
                    </div>
                    <div class="w-full md:w-1/3">
                        <label for="toggle-success" class="toggle toggle-success group reverse">
                            <span class="label">{{ trans('jobs.status') }}</span>
                            <input class="sr-only toggle-input peer" id="toggle-success" name="status" type="checkbox" checked="">
            ...
            <form action="{{route('jobs.update','test')}}" method="post">
                ...
                <div class="flex flex-col gap-1 items-center w-full md:flex-row md:gap-2">
                    ...
                    <div class="w-full md:w-1/3">
                        <input class="input" id="job_name" name="job_name" type="text" value="{{$job->title}}">
                    </div>
                    <div class="w-full md:w-1/3">
                        <label for="toggle-success" class="toggle toggle-success group reverse">
                            <span class="label">{{ trans('jobs.status') }}</span>
                            <input class="sr-only toggle-input peer" id="toggle-success" name="status" type="checkbox" @checked($job->status==0)>
                            <div class="toggle-body"></div>
                        </label>
                    </div>
                    <div class="w-full md:w-1/3">
                        ...
                        <tbody>
                            @forelse ($jobs as $job)
                                <tr>
                                    ...
                                    <td>{{ $job->title }}</td>
                                    <td>{{ $job->creator->first_name }}</td>
                                    <td>{{ $job->created_at->format('Y-m-d') }}</td>
                                    <td>
                                        <div
                                            class="badge {{ $job->status->color() }}">
                                            {{ $job->status->lang() }}
                                        </div>
                                    </td>

                                    <td>{{ ($job->updator)?$job->updator->first_name:'' }}</td>
