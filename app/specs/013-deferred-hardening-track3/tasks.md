...
**Purpose**: Close gaps remaining after the Phase 11 implementation, surfaced by the `/speckit.converge` re-assessment (2026-08-29). Both items trace to requirements the Phase 11 pass did not cover; neither is blocked on 012.

**Ordering**: CRITICAL/HIGH first; each item traces to its source requirement and gap type.

- [ ] T049 [US1] Fix the order↔stock-movement reference mapping so an order's transaction history actually resolves: `InventoryOrder::transactions()` is a `morphMany(InventoryTransaction::class, 'reference')` requiring the morph class (`App\Models\Inventory\InventoryOrder`) in `reference_type`, but `InventoryOrderService::createOrderItems` and `InventoryTransactionService::stockIn/stockOut` write the plain string `'order'` (no `morphMap` registered), so the relation is always empty; write the resolvable morph class (or register a `morphMap` mapping `order` → `InventoryOrder`) and add the four-behavior end-to-end `Track3_InventoryEndToEndTest` (type preserved + zero/false persists + reference resolves) that asserts `$order->transactions` returns rows, per FR-INV-4 / US1 AC4 / `T009` (contradicts) in `app/Services/Inventory/InventoryOrderService.php`, `app/Services/Inventory/InventoryTransactionService.php`, `app/Models/Inventory/InventoryOrder.php`, and `app/tests/Feature/Inventory/Track3_InventoryEndToEndTest.php`
- [ ] T050 [P] [US3] Scope and paginate the dashboard user list: `HomeController::index()` runs `$data['users'] = User::query()->get()` — an unpaginated, school-unscoped query returning every school's users for any authenticated user; scope it to the acting school for non-super-admins (and paginate) and add `SchoolBoundariesTest` asserting the dashboard user list is school-scoped and a null-school non-super-admin sees no cross-school data, per FR-SCOPE-2 / US3 AC2 / `T015` (contradicts; reconcile with `012 T033`/`012 T075`/`012 T032`) in `app/app/Http/Controllers/HomeController.php` and `app/tests/Feature/MultiTenancy/SchoolBoundariesTest.php`
