...
@if (Auth::user()->hasAnyPermission(['stocks-index', 'orders-index', 'order_out-index', 'stocks-inventory_order-index']))
    ...
    <div x-data="{ stocksOpen: {{ request()->is('*inventory/items/stock*') || request()->is('*inventory/orders*') ? 'true' : 'false' }} }" x-show="!search || '{{ trans('Sidebar.stocks') }}'.includes(search)">
        ...
        <div x-show="stocksOpen" class="mt-1 mx-2 space-y-1 bg-gray-800 rounded-lg overflow-hidden" style="display: none;">
            @can('stocks-index')
                <a href="{{ route('inventory.items.index', ['type' => 'stock']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/items/stock*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('Sidebar.stocks_show') }}</a>
            @endcan
            @can('orders-index')
                <a href="{{ route('inventory.orders.index', ['type' => 'purchases']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/orders/purchases*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('orders.income') }}</a>
            @endcan
            @can('order_out-index')
                <a href="{{ route('inventory.orders.index', ['type' => 'sales']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/orders/sales*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('stock.outcome_order') }}</a>
            @endcan
            @can('stocks-inventory_order-index')
                <a href="{{ route('inventory.orders.index', ['type' => 'inventory']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/orders/inventory*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('stock.inventory_order') }}</a>
            @endcan
        </div>
    </div>
@endif
...
@if (Auth::user()->hasAnyPermission(['clothes-income_order', 'clothes-index', 'clothes-outcome_order', 'clothes-inventory_order']))
    ...
    <div x-data="{ clothesOpen: {{ request()->is('*inventory/items/clothe*') ? 'true' : 'false' }} }" x-show="!search || '{{ trans('stock.clothes') }}'.includes(search)">
        ...
        <div x-show="clothesOpen" class="mt-1 mx-2 space-y-1 bg-gray-800 rounded-lg overflow-hidden" style="display: none;">
            @can('clothes-index')
                <a href="{{ route('inventory.items.index', ['type' => 'clothe']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/items/clothe*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('Sidebar.clothes_show') }}</a>
            @endcan
    ...
    <div x-data="{ booksOpen: {{ request()->is('*inventory/items/book*') ? 'true' : 'false' }} }" x-show="!search || '{{ trans('Sidebar.books_sheets') }}'.includes(search)">
        ...
        <div x-show="booksOpen" class="mt-1 mx-2 space-y-1 bg-gray-800 rounded-lg overflow-hidden" style="display: none;">
            @can('books_sheets-index')
                <a href="{{ route('inventory.items.index', ['type' => 'book']) }}" class="block px-4 py-2 text-sm transition-colors {{ request()->is('*inventory/items/book*') ? 'bg-primary/10 text-primary' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">{{ trans('Sidebar.books_sheets_show') }}</a>
            @endcan
