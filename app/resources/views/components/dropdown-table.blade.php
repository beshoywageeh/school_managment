...
@php
    $triggerClasses =
        'inline-flex items-center gap-1.5 h-8 px-3 bg-success hover:bg-success/90 active:bg-success/80 text-white text-sm font-medium rounded-lg transition-colors select-none cursor-pointer';
    $chevronClasses = 'w-3.5 h-3.5 transition-transform duration-200';
    $menuClasses =
        'absolute right-0 mt-1 min-w-[180px] bg-white border-gray-200 rounded-xl z-50 p-1 origin-top-right shadow-lg';
...
<div class="relative inline-block" x-data="{ open: false }" x-on:click.outside="open = false">
    <button type="button" x-on:click="open = !open" :aria-expanded="open" class="{{ $triggerClasses }}">
        {{ $buttonText }}
        <svg x-bind:class="open ? 'rotate-180' : ''" class="{{ $chevronClasses }}" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
                ...
                @can($item['can'] ?? '')
                    ...
                    @if (($item['type'] ?? 'link') === 'delete')
                        <form action="{{ $item['url'] }}" method="POST" class="w-full" x-on:submit="open = false">
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="{{ $itemBase }} {{ $itemColor }} {{ $item['className'] ?? '' }}"
                                {!! $itemAttrs !!}>
                                @if ($hasIcon)
