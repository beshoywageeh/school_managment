...
<div class="relative inline-block" x-data="{ open: false }" x-on:click.outside="open = false">
    ...
    <button type="button" x-on:click="open = !open" :aria-expanded="open"
        class="inline-flex items-center gap-1.5 h-8 px-3 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white text-sm font-medium rounded-lg transition-colors select-none">
        {{ $buttonText }}
    ...
    <div x-show="open" x-transition
        class="absolute right-0 mt-1 min-w-[180px] bg-white  border-gray-200  rounded-xl z-50 p-1 origin-top-right"
        style="display: none;">

        @foreach ($items as $item)
            @can($item['can'])
                @php $isDanger = isset($item['className']) && str_contains($item['className'], 'danger') || str_contains($item['className'] ?? '', 'delete') || str_contains($item['className'] ?? '', 'red'); @endphp
                ...
                @if ($item['type'] === 'button')
                    <button type="button" x-on:click="open = false"
                        ...
                        class="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg text-right transition-colors
                            ...
                            {{ $item['className'] ?? '' }}">
                        @if (isset($item['icon']))
                            <i class="{{ $item['icon'] }} text-base {{ $isDanger ? 'text-red-500' : 'text-gray-400' }}"
                                aria-hidden="true"></i>
                        @endif
                        {{ $item['text'] }}
                    </button>
                @else
                    <a href="{{ $item['url'] }}" x-on:click="open = false"
                        ...
                        class="flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors
                            ...
                            {{ $item['className'] ?? '' }}">
                        @if (isset($item['icon']))
                            <x-hero-icon name="{{ $item['icon'] }}" class="text-gray-400 text-base w-5 h-5"
                                aria-hidden="true" />
                        @endif
                        {{ $item['text'] }}
                    </a>
                ...
                {{-- Separator قبل الـ danger items --}}
                @if ($isDanger)
