<header class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 shrink-0"
    ...
    <div class="flex items-center gap-4" style="display: flex !important;">
        <button type="button" id="sidebar-toggle"
            class="lg:hidden p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg">
            <svg class="w-6 !important h-6 !important" fill="none" viewBox="0 0 24 24" stroke="currentColor"
    ...
    <div class="flex items-center gap-3"
        style="display: flex !important; align-items: center !important; flex-direction: row !important; flex-wrap: nowrap !important;">
        @auth
        ...
        @endauth
        <span class="px-3 py-1.5 bg-blue-100 text-blue-800 rounded-lg text-sm" id="datetime"
            style="display: inline-block !important; white-space: nowrap !important;">{{ now()->format('Y-m-d H:i') }}</span>
        ...
        <a href="{{ route('profile.edit') }}" class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg"
            title="Settings" style="display: inline-block !important;">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <form method="POST" action="{{ route('logout') }}" class="flex items-center gap-2"
            ...
            @csrf
            <button type="submit"
                class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg flex items-center gap-2 transition-colors"
                title="Logout" style="display: inline-flex !important;">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
