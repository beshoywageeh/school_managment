...
@section('content')
    <div x-data="{
        ...
        <div class="p-4 border-b border-gray-100 flex justify-between items-center">
            <h4 class="text-lg font-semibold text-gray-800">{{ trans('jobs.title') }}</h4>
            @can('jobs-create')
                <button type="button" class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 flex items-center gap-2" @click="window.dispatchEvent(new Event('open-modal-create-job'))">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        @can('jobs-info')
                                        <a class="show_jobs px-3 py-1 bg-blue-500 text-white text-sm rounded-lg hover:bg-blue-600" href="{{ route('jobs.show', $job->value) }}" @click.prevent="fetchJobs($el.href)">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        <button x-show="canEdit" @click="editJob(job)" class="p-2 text-yellow-600 hover:bg-yellow-50 rounded-lg" title="{{ trans('general.edit') }}">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </button>
                                        <a x-show="canDelete" :href="`{{ route('jobs.destroy', '') }}/${job.id}`" class="p-2 text-red-600 hover:bg-red-50 rounded-lg" onclick="confirmation(event)" title="{{ trans('general.delete') }}">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
