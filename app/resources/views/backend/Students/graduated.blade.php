                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        @can('Student-restore')
                                            <form action="{{ route('students.restore', $student->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="px-3 py-1 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90">
                                        ...
                                        @endcan
                                        @can('Students-delete')
                                            <form action="{{ route('students.forceDelete', $student->id) }}" method="POST" class="inline" onsubmit="return confirmation(event)">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="px-3 py-1 bg-danger text-white rounded-lg text-sm font-medium hover:bg-danger/90">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
