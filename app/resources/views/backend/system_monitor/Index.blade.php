    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-6"
        ...
        <form method="GET" x-show="open" x-transition class="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <select name="user_id" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all tom-select">
            ...
            <div>
                ...
                <input type="text" name="action" value="{{ request('action') }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="{{ trans('general.search') }}">
            </div>
            <div>
                <x-input-label>{{ trans('general.from') }}</x-input-label>
                <input type="date" name="from_date" value="{{ request('from_date') }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
            </div>
            <div>
                <x-input-label>{{ trans('general.to') }}</x-input-label>
                <input type="date" name="to_date" value="{{ request('to_date') }}" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
            </div>
            <div class="md:col-span-4 flex items-center gap-2">
                <button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 text-sm font-medium">
                    {{ trans('general.search') }}
                </button>
