...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex justify-between items-center">
            <h4 class="text-lg font-bold text-gray-800">{{ trans('Grades.title') }}</h4>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-around gap-2">
                                    <a href="{{ route('grade.show', $grade->id) }}" class="text-primary hover:bg-primary/10 rounded-lg" title="{{ trans('general.info') }}">
...
<div>
                                    ...
                                    <form action="{{ route('grade.destroy', $grade->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-danger hover:bg-danger/10 rounded-lg" onclick="confirmation(event)" title="{{ trans('general.delete') }}">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
