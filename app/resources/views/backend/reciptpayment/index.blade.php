...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100">
            <h4 class="text-lg font-bold text-gray-800">{{ trans('Recipt_Payments.title') }}</h4>
        </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @can('ReceiptPayment-info')
                                    <a href="{{ route('receipt-payment.show', $ReceiptPayment->id) }}" target="_blank" class="p-2 text-primary hover:bg-primary/10 rounded-lg" title="{{ trans('general.info') }}">
                                    ...
                                    <a href="{{ route('receipt-payment.edit', $ReceiptPayment->id) }}" class="p-2 text-primary hover:bg-primary/10 rounded-lg" title="{{ trans('general.edit') }}">
                                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    ...
                                    <form action="{{ route('receipt-payment.destroy', $ReceiptPayment->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
