...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('bookSheetsOrder.store_tawreed') }}" method="post" autocomplete="off"
              x-data="tawreed({{ $books_sheets->count() }})">
            <div class="p-6">
                @csrf
                            ...
                            @foreach ($books_sheets as $clothe)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2">
                                        ...
                                        <span class="text-gray-800 font-medium">{{ $clothe->name }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="number" value="0" name="qty[]" x-model="items[{{ $loop->index }}].qty"
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="number" value="0" name="sales[]" x-model="items[{{ $loop->index }}].sales"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                                    </td>
                                    <td class="px-4 py-2">
                                        <input type="text" disabled
                                            x-bind:value="(Number(items[{{ $loop->index }}].qty) || 0) * (Number(items[{{ $loop->index }}].sales) || 0)"
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                ...
                                <td colspan="2" class="px-4 py-2">
                                    <input type="text" disabled value="0" x-bind:value="totalQty"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600">
                                </td>
                                <td></td>
                                <td colspan="2" class="px-4 py-2 text-right font-medium text-gray-700">{{ trans('clothes.total_price') }}</td>
                                <td colspan="2" class="px-4 py-2">
                                    <input type="text" disabled value="0" x-bind:value="totalPrice"
                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600">
                                </td>
                            </tr>
                        </tfoot>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('tawreed', (count) => ({
                    items: Array.from({ length: count }, () => ({ qty: 0, sales: 0 })),
                    get totalQty() {
                        return this.items.reduce((sum, item) => sum + (Number(item.qty) || 0), 0);
                    },
                    get totalPrice() {
                        const total = this.items.reduce((sum, item) => sum + ((Number(item.qty) || 0) * (Number(item.sales) || 0)), 0);
                        return total.toLocaleString('en-EG', { style: 'currency', currency: 'EGP' });
                    }
                }));
            });
        </script>
