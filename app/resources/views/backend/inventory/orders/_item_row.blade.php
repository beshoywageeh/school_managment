<tr x-data="{ itemType: '' }">
    <td class="px-4 py-2">
        <select x-model="itemType" :name="`items[${index}][itemable_type]`" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none">
            <option value="App\Models\Inventory\InventoryItem">{{ trans('inventory.title') }}</option>
        </select>
    </td>
    <td class="px-4 py-2">
        <select :name="`items[${index}][itemable_id]`" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary outline-none tom-select">
    ...
    <td class="px-4 py-2 text-center">
        <button type="button" x-on:click="items.splice(index, 1)" class="p-1.5 text-danger hover:bg-danger/10 rounded-lg transition-colors">
