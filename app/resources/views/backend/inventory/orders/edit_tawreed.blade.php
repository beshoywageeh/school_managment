@extends('layouts.app')
@section('title')
    {{ trans('inventory.edit_item') }} - {{ trans('stock.income_order') }}
@endsection

@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100">
            <h4 class="text-lg font-semibold text-gray-800">{{ trans('stock.income_order') }}: {{ $order->order_number }}</h4>
        </div>
        ...
        <form action="{{ route('inventory.orders.update') }}" method="POST">
            ...
            <div class="p-6">
                ...
                <div x-data="{ items: {{ $order->items->toJson() }} }" class="mb-6">
                        ...
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="number" step="0.01" name="items[{{ $loop->index }}][quantity_in]" value="{{ $existingItem->quantity_in }}" class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
                                    </td>
