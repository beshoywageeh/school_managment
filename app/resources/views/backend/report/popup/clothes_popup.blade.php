<x-modal id="clothes_stock" title="{{ trans('report.clothe_stock') }}" titleButton="{{ trans('general.open') }}" can="clothes-index">
    ...
    <x-slot name="footer">
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="button" x-on:click="document.getElementById('clothes_stock').submit(); open = false;" class="px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors">{{ trans('general.show') }}</button>
        </div>
    </x-slot>
</x-modal>