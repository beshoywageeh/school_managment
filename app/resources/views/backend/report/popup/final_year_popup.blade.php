<x-modal id="final_year" title="{{ trans('report.finalyear.title') }}" titleButton="{{ trans('general.open') }}" can="fee_invoice-list">
    <form action="{{ route('report.final_year') }}" method="post" id="final_year">
        ...
        <div class="space-y-4" x-data="{
            ...
            async loadClassrooms() {
                this.classrooms = [];
                if (!this.gradeId || this.gradeId.length === 0) return;
                const map = new Map();
                ...
                this.classrooms = [...map.values()];
            }
        }">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ trans('general.choose', ['value' => trans('Grades.name')]) }}</label>
                <select name="grade[]" x-model="gradeId" @change="loadClassrooms()" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" multiple x-init="new TomSelect($el, { plugins: ['remove_button'] })">
            ...
            <div>
                ...
                <select name="classroom[]" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" multiple x-init="new TomSelect($el, { plugins: ['remove_button'] })">
                    <option value="0">{{ trans('general.all') }}</option>
    ...
    <x-slot name="footer">
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="button" x-on:click="document.getElementById('final_year').submit(); open = false;" class="px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors">{{ trans('general.show') }}</button>
        </div>
    </x-slot>
</x-modal>