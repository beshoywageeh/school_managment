<div id="final_year-modal" x-data="{ open: false }" x-cloak>
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="open = false">
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-lg mx-auto z-10">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">{{ trans('report.finalyear.title') }}</h3>
            ...
            <form action="{{ route('report.final_year') }}" method="post">
                <div class="p-6 space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ trans('general.choose', ['value' => trans('Grades.name')]) }}</label>
                        <select name="grade[]" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all grades select2" multiple>
                    ...
                    <div>
                        ...
                        <select name="classroom[]" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all classrooms select2" multiple>
                            <option value="0">{{ trans('general.all') }}</option>
                        </select>
                    </div>
                </div>
