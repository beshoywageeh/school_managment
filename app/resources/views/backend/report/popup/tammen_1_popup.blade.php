<x-modal id="tammen_1" title="{{ trans('report.student_tammen') }}" titleButton="{{ trans('general.open') }}" can="Students-list">
    ...
    <x-slot name="footer">
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="button" x-on:click="document.getElementById('tammen_1').submit(); open = false;" class="px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors">{{ trans('general.show') }}</button>
        </div>
    </x-slot>
</x-modal>