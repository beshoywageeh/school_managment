<header role="banner"
    class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 shrink-0"
    style="border-color: {{ $brandColor ?: 'var(--module-primary)' }};">
    <div class="flex items-center gap-4">
        ...
        <button type="button"
            x-on:click="sidebarPinned = !sidebarPinned; localStorage.setItem('sidebarPinned', sidebarPinned)"
        ...
        @if ($moduleTitle)
            <span class="font-semibold text-gray-600 text-sm border-s-2 border-gray-300 ps-3">{{ $moduleTitle }}</span>
        @endif
    </div>
    ...
    <div class="flex items-center gap-3">
        ...
        <button type="button" id="btnFullscreen"
            ...
            title="{{ trans('general.fullscreen') ?? 'Fullscreen' }}">
        ...
        <a href="{{ route('profile.edit') }}"
            class="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg"
            title="{{ trans('general.settings') ?? 'Settings' }}">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <form method="POST" action="{{ route('logout') }}" class="flex items-center gap-2">
            ...
            <button type="submit"
                ...
                title="{{ trans('general.logout') ?? 'Logout' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
