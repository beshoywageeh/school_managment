...
class ClothesOrderController extends Controller
{
    ...
    public function index($type)
    {
        ...
        $validTypes = ['sales', 'inventory', 'gard'];
        if (! in_array($type, $validTypes)) {
            abort(404);
        }
        $orders = clothes_order::where('school_id', $school->id)
            ->where('type', $type)
            ->with(['stocks'])
            ->get();
    ...
    public function tawreed()
    {
        ...
        $generate_code = clothes_order::where('type', 'inventory')
    ...
    public function clothes_order_gard_edit($id)
    {
        ...
        try {
            $order = clothes_order::where('id', $id)
                ->with('stocks')
                ->first();
            $type = 'gard';
            ...
            // return $order;
            return view(
                'backend.clothes_order.inventory_edit',
                get_defined_vars(),
            );
        } catch (\Exception $e) {
    ...
    {
        try {
            $order = clothes_order::where('id', $request->id)
                ->with('stocks')
                ->first();
            $type = 'gard';
            foreach ($request->stock_id as $key => $stock) {
                ...
                $current_qty = $stock
                    ->orders()
                    ->where('clothes_orders.id', '!=', $order->id)
                    ->where('clothes_id', $stock->id);
                $qty =
                    $request->actual_stock[$key] -
                    ($stock->opening_qty +
                        $current_qty->sum('quantity_in') -
                        $current_qty->sum('quantity_out'));
                if ($qty < 0) {
                    $order
                        ->stocks()
                        ->syncWithPivotValues('clothes_orders.id', [
                            'clothes_id' => $stock->id,
                ...
                $this->logActivity(
                    trans('log.clothes_order.stock_updated_action'),
                    trans('log.clothes_order.stock_updated', [
                        'stock_name' => $stock->name,
                        'order_number' => $order->auto_number,
                    ]),
                );
            }

            return redirect()
                ->route('clothes.index', 'gard')
    ...
    public function clothes_stock_submit(Request $request)
    {
        ...
        try {
            ...
            $this->logActivity(
                trans('log.clothes_order.supply_added_action'),
                trans('log.clothes_order.supply_added', [
                    'order_id' => $order->id,
                ]),
            );
            \DB::commit();
            ...
            return redirect()
                ->route('clothes-order.index', 'inventory')
                ->with('success', trans('general.success'));
        } catch (\Exception $e) {
            \DB::rollback();
            \Log::error(
                'Error submitting clothes stock: '.$e->getMessage(),
    ...
    public function clothes_stock_update(Request $request)
    {
        try {
            ...
            $this->logActivity(
                trans('log.clothes_order.supply_updated_action'),
                trans('log.clothes_order.supply_updated', [
                    'order_number' => $order->auto_number,
                ]),
            );
            \DB::commit();
    ...
    public function destroy($id)
    {
        try {
            ...
            $this->logActivity(
                trans('log.clothes_order.supply_deleted_action'),
                trans('log.clothes_order.supply_deleted', [
                    'order_number' => $order->auto_number,
                ]),
            );
            $order->delete();
        ...
        try {
            ...
            $this->logActivity(
                ...
                trans('log.clothes_order.out_order_added', [
                    'order_id' => $order->id,
                    'student_name' => Student::findorfail(
                        $request->student_id,
                    )->name,
                ]),
            );
            ...
            return redirect()
                ->route('clothes_order.print', $order->id)
                ->with('success', trans('general.success'));
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    ...
    public function clothes_out_order_print($id)
    {
        try {
            $order = clothes_order::where('id', $id)
                ->with('stocks', 'students')
                ->first();
            $school = $this->getSchool();

            return view(
                'backend.clothes_order.out_order_print',
                compact('order'),
            );
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
    ...
    public function clothes_out_order_edit($id)
    {
        try {
            ...
            return view(
                'backend.clothes_order.out_order_edit',
                get_defined_vars(),
            );
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
    ...
    public function clothes_out_order_update(Request $request)
    {
        try {
            ...
            $this->logActivity(
                trans('log.clothes_order.out_order_updated_action'),
                trans('log.clothes_order.out_order_updated', [
                    'order_id' => $order->id,
                    'student_name' => Student::findorfail(
                        $request->student_id,
                    )->name,
    ...
    public function clothes_order_gard()
    {
        try {
            ...
            $generate_code = clothes_order::where('type', '3')
                ->orderBy('auto_number', 'desc')
                ->first();
            $order = clothes_order::create([
                'auto_number' => isset($generate_code)
            ...
            $this->logActivity(
                trans('log.clothes_order.inventory_order_added_action'),
                trans('log.clothes_order.inventory_order_added', [
                    'order_number' => $order->auto_number,
                ]),
            );

            return view(
                'backend.clothes_order.gard_create',
                compact('order', 'stocks'),
            );
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    ...
    public function clothes_order_gard_submit(Request $request)
    {
        try {
            foreach ($request->stock_id as $key => $stock) {
                ...
                $this->logActivity(
                    trans(
                        'log.clothes_order.stock_added_to_inventory_action',
                    ),
                    trans('log.clothes_order.stock_added_to_inventory', [
                        'stock_name' => $stock->name,
                        'inventory_number' => $request->id,
                    ]),
                );
            }
    ...
    public function pay($id)
    {
        try {
            ...
            $this->logActivity(
                trans('log.clothes_order.receipt_updated_to_paid_action'),
                trans('log.clothes_order.receipt_updated_to_paid', [
                    'order_number' => $order->auto_number,
                ]),
            );

            return redirect()
                ->route('clothes_order.index', ['type' => 'sales'])
                ->with('info', 'تم الدفع بنجاح');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    }
    ...
    public function getClothes($id)
    {
        ...
        $clothes = clothes::where('grade_id', $std->grade_id)
            ->where('classroom_id', $std->classroom_id)
            ->get();
